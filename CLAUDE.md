...
| 코드블록 (SQL) | `<div>` 기반 하이라이팅 — 키워드 파란색(`#79c0ff`), 함수 주황색(`#ffa657`), 주석 회색(`#8b949e`), 기본 흰색(`#ffffff`) |
| 표 | 다크 헤더 + 홀짝 행 배경 + 호버 효과 |
| 인용구 | 파란 왼쪽 선 + 하늘색 배경 |
| 추천 해시태그: | 주황 칩 박스 |

> **SQL 코드블록 구현 방식**: TinyMCE가 `<pre><code>` 내부의 `<span>`을 이스케이프하는 문제로,
> `_sql_block_to_div()` 함수에서 `<div>` 컨테이너 + 라인별 `<span style="display:block">` 구조로 우회 처리.
...
### 포스팅 원고 작성 규칙
- **제목 형식**: `[카테고리] 제목 내용` 형식으로 시작 (전체 35자 이내)
